...
root_dir = '/media/weixing/diskD/dataset/mpii/mpii_human_pose_v1_u12_2'
mat_file = osp.join(root_dir, 'mpii_human_pose_v1_u12_1.mat')
db_type = 'train'  # train, test
annot_file = loadmat(mat_file)['RELEASE']
save_path = root_dir + '/' + '../annotations/' + db_type + '.json'
...
for img_id in range(img_num):
    ...
    if ((db_type == 'train' and annot_file['img_train'][0][0][0][img_id] == 1) or (
            db_type == 'test' and annot_file['img_train'][0][0][0][img_id] == 0)) and \
        ...
        image_path = osp.join(root_dir, '..', filename)
        ...
        for pid in range(person_num):
            ...
            if check_empty(annot_file['annolist'][0][0][0][img_id]['annorect'][0][pid],
                           'annopoints') == False:  # kps is annotated
                ...
                person_dict = {'id': aid, 'image_id': img_id, 'category_id': 1, 'area': bbox[2] * bbox[3],
                               'bbox': bbox.tolist(), 'iscrowd': 0, 'keypoints': kps.reshape(-1).tolist(),
