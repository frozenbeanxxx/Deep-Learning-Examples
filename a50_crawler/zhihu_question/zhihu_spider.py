...
class zhihu_answer():
    ...
    def get_total(self,question_id):
        url = "https://www.zhihu.com/api/" \
              ...
              "2Cvoting%2Cis_thanked%2Cis_nothelp%2Cis_recognized%3Bdata%5B*%5D.mark_infos%5B*%5D" \
              ".url%3Bdata%5B*%5D.author.follower_count%2Cbadge%5B*%5D.topics%3Bdata%5B*%5D.settings." \
              "table_of_content.enabled&offset=&limit={limit}&sort_by=default&platform=desktop".format(question_id=str(question_id), limit=20)
        response=requests.get(url,headers=self.headers)
        time.sleep(3)
        json_result = json.loads(response.content)
    ...
    def single_answer(self,question_id):
        # get_following_url("c1afdd1905bff36b178575ac23461180")
        ...
        try:
            with open(question_title+".txt",mode="w",encoding='utf-8') as f:
    ...
    def get_next_question(self,question):
        url="https://www.zhihu.com/api/v4/questions/{question_id}/similar-questions?include=data%5B*%5D.answer_count%2Cauthor%2Cfollower_count&limit=5".format(question_id=question)
        # print(url)
        response = requests.get(url,headers=self.headers)
        # print(response.text)
        json_result = json.loads(response.content)
